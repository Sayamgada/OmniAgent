...
export const WorkflowNodeCard = memo(({ data, selected }: NodeProps) => {
  ...
  return (
    <div
      className={cn(
        "group relative w-[230px] rounded-xl border bg-card p-3 shadow-md transition-all duration-200",
        styles.border,
    ...
    >
      ...
      <div className="mb-2 flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className={cn("rounded px-1.5 py-0.5 font-mono text-[9px] uppercase tracking-wide", styles.badge)}>
            {detail.type.replace("_", " ")}
          </span>
        </div>
      </div>

      <h4 className="truncate text-xs font-bold text-foreground">{detail.label}</h4>
      {detail.subtitle && (
        <p className="mt-0.5 truncate text-[11px] font-medium text-primary">{detail.subtitle}</p>
      )}
      {detail.preview && (
