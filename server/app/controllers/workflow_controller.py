...
async def generate_workflow(domain: str, description: str, top_k: int = 5) -> Dict[str, Any]:
    """
    Coordinates searching automations (similarity logic) 
    and generating the workflow (planner/workflow logic) using LLM Service.
    """
    # 1. Similarity Logic / Context gathering
    docs = search_automations(query=description, category=domain, top_k=top_k)
    context = "\n\n".join(doc.page_content for doc in docs) if docs else ""
    
    # 2. Workflow generation logic (Prompt formulation)
    user_content = (
        f"domain: {domain}\n"
    ...
    # 3. Invoking the generic LLM Service (responsible ONLY for Grok communication)
    return await generate_json_response(messages)
